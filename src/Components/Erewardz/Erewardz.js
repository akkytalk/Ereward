...
function Erewardz(props) {
  //token
  const token = props.login?.login[0]?.token?.token;

  useEffect(() => {
    props.erewardzGetData(token);
  }, []);
        ...
        <Card>
          ...
          <CardBody>
            ...
            >
              ...
              <tbody>
                {props?.erewardz?.isLoading ? (
                  <tr>
                    <td colSpan={12}>
                      <Loader color={"primary"} />
                    </td>
                  </tr>
                ) : props?.erewardz?.erewardz?.length > 0 ? (
                  props?.erewardz?.erewardz
                    ?.filter(
                      (employee) =>
                        employee?.name
                          ?.toLowerCase()
...
const mapStateToProps = (state) => {
  return {
    login: state.login,
    erewardz: state.erewardz,
  };
};

const mapDispatchToProps = (dispatch) => {
  return {
    erewardzGetData: (token) => dispatch(actions.erewardzGetData(token)),
  };
};
export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(Erewardz)
);
