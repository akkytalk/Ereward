        ...
        <Card>
          ...
          <CardBody>
            ...
            >
              ...
              <tbody>
                {props?.septUsers?.isLoading ? (
                  <tr>
                    <td colSpan={12}>
                      <Loader color={"primary"} />
                    </td>
                  </tr>
                ) : props?.septUsers?.septUsers?.length > 0 ? (
                  props?.septUsers?.septUsers
                    ?.filter(
                      (employee) =>
                        employee?.name
                          ?.toLowerCase()
...
const mapStateToProps = (state) => {
  return {
    login: state.login,
    septUsers: state.septUsers,
  };
};

const mapDispatchToProps = (dispatch) => {
  return {
    septUsersGetData: (token) => dispatch(actions.septUsersGetData(token)),
  };
};
export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(September)
);
