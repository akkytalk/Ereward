...
function WhiteUsers(props) {
  //token
  const token = props.login?.login[0]?.token?.token;

  useEffect(() => {
    props.whiteUsersGetData(token);
  }, []);
        ...
        <Card>
          ...
          <CardBody>
            ...
            >
              ...
              <tbody>
                {props?.whiteUsers?.isLoading ? (
                  <tr>
                    <td colSpan={12}>
                      <Loader color={"primary"} />
                    </td>
                  </tr>
                ) : props?.whiteUsers?.whiteUsers?.length > 0 ? (
                  props?.whiteUsers?.whiteUsers
                    ?.filter(
                      (employee) =>
                        employee?.name
                          ?.toLowerCase()
...
const mapStateToProps = (state) => {
  return {
    login: state.login,
    whiteUsers: state.whiteUsers,
  };
};

const mapDispatchToProps = (dispatch) => {
  return {
    whiteUsersGetData: (token) => dispatch(actions.whiteUsersGetData(token)),
  };
};
export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(WhiteUsers)
);
