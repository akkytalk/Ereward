...
function SepImpactPro(props) {
  //token
  const token = props.login?.login[0]?.token?.token;

  useEffect(() => {
    props.sepProGetData(token);
  }, []);
      ...
      <div className="container-fluid">
        <Card>
          <CardHeader className="bg-info text-white">
            <Row>
              <Col>
                <strong>September Impact Pro</strong>
              </Col>
              <Col md={5}></Col>
              <Col md={2}>
          ...
          <CardBody>
            ...
            >
              ...
              <tbody>
                {props?.sepPro?.isLoading ? (
                  <tr>
                    <td colSpan={12}>
                      <Loader color={"primary"} />
                    </td>
                  </tr>
                ) : props?.sepPro?.sepPro?.length > 0 ? (
                  props?.sepPro?.sepPro
                    ?.filter(
                      (employee) =>
                        employee?.name
...
const mapDispatchToProps = (dispatch) => {
  return {
    sepProGetData: (token) => dispatch(actions.sepProGetData(token)),
  };
};
export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(SepImpactPro)
);
