...
function LapUsers(props) {
  //token
  const token = props.login?.login[0]?.token?.token;

  useEffect(() => {
    props.lapUsersGetData(token);
  }, []);
        ...
        <Card>
          ...
          <CardBody>
            ...
            >
              ...
              <tbody>
                {props?.lapUsers?.isLoading ? (
                  <tr>
                    <td colSpan={12}>
                      <Loader color={"primary"} />
                    </td>
                  </tr>
                ) : props?.lapUsers?.lapUsers?.length > 0 ? (
                  props?.lapUsers?.lapUsers
                    ?.filter(
                      (employee) =>
                        employee?.name
                          ?.toLowerCase()
...
const mapStateToProps = (state) => {
  return {
    login: state.login,
    lapUsers: state.lapUsers,
  };
};

const mapDispatchToProps = (dispatch) => {
  return {
    lapUsersGetData: (token) => dispatch(actions.lapUsersGetData(token)),
  };
};
export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(LapUsers)
);
