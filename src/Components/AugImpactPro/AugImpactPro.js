...
function AugImpactPro(props) {
  //token
  const token = props.login?.login[0]?.token?.token;

  useEffect(() => {
    props.augProGetData(token);
  }, []);
        ...
        <Card>
          ...
          <CardBody>
            ...
            >
              ...
              <tbody>
                {props?.augPro?.isLoading ? (
                  <tr>
                    <td colSpan={12}>
                      <Loader color={"primary"} />
                    </td>
                  </tr>
                ) : props?.augPro?.augPro?.length > 0 ? (
                  props?.augPro?.augPro
                    ?.filter(
                      (employee) =>
                        employee?.name
...
const mapDispatchToProps = (dispatch) => {
  return {
    augProGetData: (token) => dispatch(actions.augProGetData(token)),
  };
};
export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(AugImpactPro)
);
