...
function RiseUsers(props) {
  //token
  const token = props.login?.login[0]?.token?.token;

  useEffect(() => {
    props.riseUsersGetData(token);
  }, []);
        ...
        <Card>
          ...
          <CardBody>
            ...
            >
              ...
              <tbody>
                {props?.riseUsers?.isLoading ? (
                  <tr>
                    <td colSpan={12}>
                      <Loader color={"primary"} />
                    </td>
                  </tr>
                ) : props?.riseUsers?.riseUsers?.length > 0 ? (
                  props?.riseUsers?.riseUsers
                    ?.filter(
                      (employee) =>
                        employee?.name
                          ?.toLowerCase()
...
const mapStateToProps = (state) => {
  return {
    login: state.login,
    riseUsers: state.riseUsers,
  };
};

const mapDispatchToProps = (dispatch) => {
  return {
    riseUsersGetData: (token) => dispatch(actions.riseUsersGetData(token)),
  };
};
export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(RiseUsers)
);
