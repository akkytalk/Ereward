...
function Gimaestro(props) {
  //token
  const token = props.login?.login[0]?.token?.token;

  useEffect(() => {
    props.gimaestroGetData(token);
  }, []);
        ...
        <Card>
          ...
          <CardBody>
            ...
            >
              ...
              <tbody>
                {props?.gimaestro?.isLoading ? (
                  <tr>
                    <td colSpan={12}>
                      <Loader color={"primary"} />
                    </td>
                  </tr>
                ) : props?.gimaestro?.gimaestro?.length > 0 ? (
                  props?.gimaestro?.gimaestro
                    ?.filter(
                      (employee) =>
                        employee?.name
                          ?.toLowerCase()
...
const mapStateToProps = (state) => {
  return {
    login: state.login,
    gimaestro: state.gimaestro,
  };
};

const mapDispatchToProps = (dispatch) => {
  return {
    gimaestroGetData: (token) => dispatch(actions.gimaestroGetData(token)),
  };
};
export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(Gimaestro)
);
