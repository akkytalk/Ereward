        ...
        <Card>
          ...
          <CardBody>
            ...
            >
              ...
              <tbody>
                {props?.powerUsers?.isLoading ? (
                  <tr>
                    <td colSpan={12}>
                      <Loader color={"primary"} />
                    </td>
                  </tr>
                ) : props?.powerUsers?.powerUsers?.length > 0 ? (
                  props?.powerUsers?.powerUsers
                    ?.filter(
                      (employee) =>
                        employee?.name
                          ?.toLowerCase()
...
const mapStateToProps = (state) => {
  return {
    login: state.login,
    powerUsers: state.powerUsers,
  };
};

const mapDispatchToProps = (dispatch) => {
  return {
    powerUsersGetData: (token) => dispatch(actions.powerUsersGetData(token)),
  };
};
export default withRouter(connect(mapStateToProps, mapDispatchToProps)(Power));
