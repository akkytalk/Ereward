...
function RiseUsers2(props) {
  //token
  const token = props.login?.login[0]?.token?.token;

  useEffect(() => {
    props.riseUsers2GetData(token);
  }, []);
        ...
        <Card>
          ...
          <CardBody>
            ...
            >
              ...
              <tbody>
                {props?.riseUsers2?.isLoading ? (
                  <tr>
                    <td colSpan={12}>
                      <Loader color={"primary"} />
                    </td>
                  </tr>
                ) : props?.riseUsers2?.riseUsers2?.length > 0 ? (
                  props?.riseUsers2?.riseUsers2
                    ?.filter(
                      (employee) =>
                        employee?.name
                          ?.toLowerCase()
...
const mapStateToProps = (state) => {
  return {
    login: state.login,
    riseUsers2: state.riseUsers2,
  };
};

const mapDispatchToProps = (dispatch) => {
  return {
    riseUsers2GetData: (token) => dispatch(actions.riseUsers2GetData(token)),
  };
};
export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(RiseUsers2)
);
